...
from torch_geometric.utils import train_test_split_edges, remove_self_loops, negative_sampling
...
for seeds in [1024, 128, 4399, 4321, 42]:
    ...
    if __name__ == '__main__':
        ...
        parser.add_argument("--upbound", type=float, default=0.80, help="Threshold for pseudo labeling")
        parser.add_argument("--top", type=int, default=50, help="Number of pseudo label in each iteration")
        parser.add_argument("--early_stop", type=int, default=2, help="stop after consecutive 0 PL iteration")
        ...
        for _ in range(K):
            ...
            adj_p = best_model(x, data.train_pos_edge_index)
            adj_pp += adj_p
            # pp_edge = adj_p[data.train_pos_edge_index[0, :], data.train_pos_edge_index[1, :]]
            # pn_edge = adj_p[data.train_negative_edges[0, :], data.train_negative_edges[1, :]]
        ...
        random_pseudo_ones = negative_sampling(data.train_negative_edges, data.x.size(0), top)
        lossr = model.loss(data.x, torch.cat((random_pseudo_ones, data.train_pos_edge_index), 1),
                           data.train_negative_edges, seed)
        ...
        logger.info("loss random pseudo: {}".format(loss_r))
